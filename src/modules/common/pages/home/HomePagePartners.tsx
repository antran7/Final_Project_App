...
const PartnersSection = () => {
  return (
    <div className="mt-24 text-center px-5 md:px-24">
      <h1 className="text-4xl font-bold">Our Partners & Alliances</h1>
      <p className="text-lg mt-4 mb-12 text-gray-700">
        We partnered with global leaders to drive innovative technology solutions with excellence
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 mb-48">
        {partners.map((partner, index) => (
          <div
            key={index}
            className="flex justify-center items-center bg-white/10 backdrop-blur-md border border-white/20 shadow-lg rounded-xl p-4 hover:border-blue-500 hover:shadow-blue-500/40 transition-all duration-300"
          >
            <img src={partner.logo} alt={partner.name} draggable="false" className="max-h-12 object-contain" />
          </div>
        ))}
      </div>
    </div>
